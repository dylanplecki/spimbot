...
Guava::Guava() : Fruit(), radius(3), color(QColor(120, 145, 50, 255))
{}

Guava::Guava(double x0, double y0, double velocity_x, double velocity_y,
    double acceleration_y, int points) : Fruit(x0, y0, velocity_x, velocity_y,
    acceleration_y, points), radius(3), color(QColor(120, 145, 50, 255))
{}

void Guava::draw(QPainter *painter)
{
    QPen pen(color);
    ...
    painter->setPen(pen);
    painter->drawEllipse(SCALE(x - radius), SCALE(y - radius),
        SCALE(2 * radius), SCALE(2 * radius));
}

void Guava::check_collision(bot_state_t *bots)
{
    if(!bots) return;
    if (smooshed == -1)
    {
